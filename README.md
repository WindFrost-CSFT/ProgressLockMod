...
这是一个能定时解锁 NPC 和事件的 Mod，旨在防止玩家在服务器中过度速通。
...
* **广泛兼容性 (NPC Compatibility)**: 完美支持所有 Mod 的 NPC。
* **事件支持 (Event Support)**: 事件目前仅支持原版 (Vanilla)。
* **Boss 特殊适配 (Boss Support)**: 完美支持多部位 Boss 和多体节 Boss 的判定。
...
**主指令 (Main Command):** `/progress` 或 `/pl`
